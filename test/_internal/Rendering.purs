...
import Test.Helpers.Rendering as Render
...
testRendering = describe "Path Rendering" $ do
  _ <- test "renders simple path with capture" do
    let result = Render.renderSimplePath (Proxy :: Proxy (Path ("users" / "id" : Int / "posts")))
    result # expectToEqual "/users/{id}/posts"

  _ <- test "renders path with query params" do
    let result = Render.renderFullPath (Proxy :: Proxy (Path ("api" / "posts") :? { page :: Int, sort :: String }))
    expectToEqual "/api/posts?page={page}&sort={sort}" result

  test "renders complex path with captures and query params" do
    let result = Render.renderFullPath (Proxy :: Proxy (Path ("api" / "users" / "id" : Int / "posts") :? { limit :: Int, offset :: Int }))
